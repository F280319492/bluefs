    ...
    while (len > 0) {
        ...
        if (off < buf->bl_off || off >= buf->get_buf_end()) {
            ...
            dout(20) << __func__ << " fetching 0x"
                     << std::hex << x_off << "~" << l << std::dec
                     << " of " << *p << dendl;
            int r = bdev->read(p->offset + x_off, l, &buf->bl, ioc[p->bdev],
                                        cct->_conf->bluefs_buffered_io);
            assert(r == 0);
        }
        ...
        if (out) {
            ...
            buf->bl.copy(out, off - buf->bl_of, r);
