...
class BlueRocksSequentialFile : public rocksdb::SequentialFile {
    ...
    BlueFS::FileReader *h;
public:
    BlueRocksSequentialFile(BlueFS *_fs, BlueFS::FileReader *_h) : fs(_fs), h(_h) {}
...
class BlueRocksRandomAccessFile : public rocksdb::RandomAccessFile {
    ...
    BlueFS::FileReader *h;
public:
    BlueRocksRandomAccessFile(BlueFS *_fs, BlueFS::FileReader *_h) : fs(_fs), h(_h) {}
...
class BlueRocksWritableFile : public rocksdb::WritableFile {
    ...
    BlueFS::FileWriter *h;
public:
    BlueRocksWritableFile(BlueFS *_fs, BlueFS::FileWriter *_h) : fs(_fs), h(_h) {}
...
public:
    ...
    BlueRocksFileLock(BlueFS *_fs, BlueFS::FileLock *l) : fs(_fs), lock(l) { }
...
{
    // ignore the filename :)
    result->reset(new BlueFSRocksdbLogger());
    return rocksdb::Status::OK();
}
...
rocksdb::Env* MkfsMount(const std::string& path, bool need_mkfs) {
    ...
    bluefs->add_block_device(path);
    if (need_mkfs) {
        uint64_t alloc_size = bluefs->cct->_conf->bluefs_alloc_size;
        bluefs->add_block_extent(alloc_size, bluefs->get_block_device_size()-alloc_size);
        bluefs->mkfs();
    }
    if (bluefs->mount() < 0) {
        return nullptr;
...
void Unmount(rocksdb::Env* env) {
    BlueFS* fs = static_cast<BlueRocksEnv*>(env)->GetFS();
    fs->umount();
    delete fs;
    delete env;
}